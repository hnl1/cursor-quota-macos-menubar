...
第三方 macOS 菜单栏小工具。读取本机已登录的 Cursor 账号，显示当前账期还剩多少额度。栏里看进度，悬停看摘要，点击看 Cursor 模型、其他模型和 Grok Bot 周额度。
...
鼠标在图标上停约 0.3 秒会弹出各池剩余、套餐额度和重置时间。点击打开明细，可刷新或退出。
...
1. 从弹窗退出应用
2. 删除 `~/Applications/Cursor Quota.app`
3. 从 `~/.cursor/hooks.json` 去掉指向 `./hooks/nudge-cursor-quota.sh` 的 `stop` / `sessionEnd`
4. 删除 `~/.cursor/hooks/nudge-cursor-quota.sh`
